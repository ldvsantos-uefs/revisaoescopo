...
TERMOS_PRIORITARIOS = {
    'ig': [
        ...
        'indicação geográfica', 'indicações geográficas',
        'indicacion geografica', 'indicaciones geograficas',
        'protected geographical indication', 'pgi',
        'protected designation of origin', 'pdo',
        'denominação de origem', 'denominación de origen',
        'denominazione di origine', 'denomination of origin',
        'indication of provenance', 'indication of source'
    ],
    'qualidade': [
        'authentication', 'origin authentication',
        'traceability', 'quality control',
        'origin verification', 'terroir'
    ]
}
...
TERMOS_ALTA = {
    'ml_algorithms': [
        ...
        'neural network', 'neural networks', 'ann',
...
TERMOS_ADEQUADOS = [
    'data mining', 'predictive model', 'ensemble learning',
...
SCORE_EXCELENCIA = 40  # >= 40 pontos
SCORE_ALTA = 30        # >= 30 pontos
SCORE_ADEQUADA = 20    # >= 20 pontos
SCORE_BAIXA = 0        # < 20 pontos (excluir)
        ...
        with open(arquivo_bib, 'r', encoding='utf-8', errors='ignore') as arquivo:
            ...
            for linha in arquivo:
                ...
                if linha.strip().startswith('@'):
                    ...
                    for campo in ['title', 'author', 'year', 'abstract', 'keywords']:
                        padrao = rf'{campo}\s*=\s*\{{(.+?)\}}[,\n]'
                ...
                if linha.strip() == '}' and ref_atual:
                    referencias[ref_atual]['conteudo_completo'] = ' '.join(conteudo_completo).lower()
                    ref_atual = None
...
def gerar_bibliografia_filtrada(resultados: Dict, arquivo_saida: str):
    ...
    with open(arquivo_saida, 'w', encoding='utf-8') as bib:
        ...
        bib.write("% ML para Indicações Geográficas\n")
        ...
        for categoria in ['excelencia', 'alta', 'adequada']:
            if resultados[categoria]:
                ...
                for chave, ref, score, termos in resultados[categoria]:
                    ...
                    for campo in ['author', 'title', 'journal', 'year', 'volume', 'pages', 'doi']:
                        valor = ref.get(campo, '')
                        if valor:

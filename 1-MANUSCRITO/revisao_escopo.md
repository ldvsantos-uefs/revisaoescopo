...
As Indicações Geográficas (IGs) constituem ativos estratégicos para a valorização de produtos com identidade territorial, cuja certificação depende de métodos confiáveis de autenticação. Embora o Aprendizado de Máquina (ML) tenha se tornado central nesse processo, falta uma síntese sistemática de suas aplicações em IGs. Esta revisão de escopo teve como objetivo mapear o estado da arte das aplicações de ML em IGs, identificando algoritmos, produtos e técnicas analíticas predominantes, bem como a estrutura conceitual do campo. Foram incluídos artigos revisados por pares que aplicaram ML à autenticação ou caracterização de produtos com IG, identificados por busca sistemática em Scopus e Web of Science (2010–2025), combinada a filtragem automatizada por pontuação semântica e avaliação de qualidade metodológica (ICC = 0,87). A síntese integrou Análise de Correspondência Múltipla, Análise de Clusters, Análise de Redes, Análise Temporal e modelagem preditiva, a partir de 148 estudos selecionados dentre 272 registros iniciais. Os resultados indicam crescimento superior a 400% nas publicações desde 2018 e organização do campo em “famílias tecnológicas” estáveis, nas quais combinações específicas de matrizes (vinhos, azeites, mel, carnes, chás), instrumentos analíticos (NIR, FTIR, LC‑MS, ICP‑MS) e algoritmos que se repetem em dez clusters funcionais. A análise de redes revelou um núcleo algorítmico denso (SVM, Random Forest, Neural Networks) conectado a módulos especializados por produto e região, enquanto a modelagem preditiva global mostrou que, embora os algoritmos atinjam acurácias de 80–100% na autenticação de origem e detecção de fraude, apresentam desempenho apenas moderado na predição de um índice sintético de qualidade/impacto dos estudos (R² ≈ 0,11–0,14; melhor modelo logístico com F1 ≈ 0,70). Observa-se transição de provas de conceito para especialização metodológica, na qual a escolha de algoritmos e plataformas analíticas passa a ser condicionada pela matriz do produto, pelo contexto geográfico e por requisitos regulatórios de interpretabilidade e validação externa. Essas evidências têm implicações diretas para o desenho de sistemas de certificação de origem na era digital, indicando tanto o potencial de ML para reforçar a robustez probatória das IGs quanto as limitações atuais para automatizar decisões de qualidade científica e regulatória.
...
![Biplot de Análise de Correspondência Múltipla (ACM) mostrando a evolução temporal das aplicações de aprendizado de máquina para autenticação de indicações geográficas (2010-2025).](2-FIGURAS/mca_biplot.png){#fig:mca_temporal width="80%"}
...
O padrão temporal revela três fases. Entre 2010 e 2018, predomina uma fase de consolidação metodológica, concentrada em produtos tradicionais europeus (especialmente vinhos) e em técnicas espectroscópicas consolidadas (NIR, FTIR) combinadas a algoritmos clássicos como PLS-DA e SVM, com relativa homogeneidade de abordagens [@Mohammadi2024; @Rebiai2022]. O período intermediário (2019–2021) marca uma transição, na qual a democratização de ferramentas de ML e o acesso ampliado a técnicas analíticas avançadas favorecem a diversificação de produtos (incluindo chás e plantas medicinais asiáticos) e a adoção gradual de algoritmos como Random Forest e redes neurais [@Liakos2018]. A fase recente (2022–2025), que concentra 64% do corpus, corresponde a uma expansão rápida e heterogênea, com crescimento das aplicações de Deep Learning, metabolômica *untargeted*, abordagens multimodais e estratégias de transfer learning [@He2024; @Liu2025; @Wang20218065; @Peng2025; @Feng2025].

A distribuição por produto demonstra que vinhos servem como referência metodológica ao longo de todo o período. Mel e azeites concentram-se no intervalo intermediário, devido à maturidade de técnicas espectrais aplicadas a essas matrizes. Café evidencia crescimento recente, associado à adoção de técnicas de alta resolução e modelagem profunda. A categoria "Other" agrupa matrizes diversas (carnes, chás, frutas, plantas medicinais), indicando ampliação do escopo de ML em Indicações Geográficas e fragmentação amostral.
...
![Comparação do desempenho de modelos de regressão e classificação para predição do score contínuo e da categoria de alto score dos estudos.](2-FIGURAS/model_metricas_comparacao.png){#fig:model_metricas width="80%"}
...
![Heatmap dos perfis de clusters de estudos, mostrando a coocorrência de produtos, instrumentos analíticos e algoritmos de machine learning em dez grupos principais.](2-FIGURAS/cluster_heatmap_profiles.png){#fig:cluster_heatmap width="80%"}
...
As tendências metodológicas identificadas refletem avanços tecnológicos e demandas regulatórias crescentes no domínio de autenticação de IGs mediante Machine Learning. As métricas de centralidade da rede de coocorrência entre produtos, instrumentos e algoritmos reforçam a leitura de que esse ecossistema é estruturado em torno de alguns núcleos algorítmicos com alta conectividade. A rede apresenta 20 nós e 58 arestas, com densidade moderada (0,30) e coeficiente de clustering elevado (0,59), indicando um sistema relativamente coeso, no qual subgrupos densamente conectados são interligados por alguns nós com papel de ponte. As maiores centralidades de grau e autovetor concentram-se em `NeuralNetwork`, `SVM` e `RandomForest`, que formam o núcleo algorítmico do campo, fortemente conectado a produtos como chás e mel e a técnicas espectroscópicas (NIR, FTIR). Por outro lado, nós como `GCMS`, `LCMS` e `Honey` apresentam alta betweenness centrality, funcionando como corredores que conectam módulos distintos da rede, especialmente entre estudos asiáticos com carnes e mel e aplicações europeias em azeites, vinhos e queijos. A detecção de comunidades pelo algoritmo de Louvain identificou três blocos principais: (i) um módulo de árvores de decisão (Random Forest, Decision Tree, Gradient Boosting) associado a vinhos, mel e estudos em África; (ii) um módulo SVM/KNN acoplado a GC-MS/LC-MS e carnes, com predominância de aplicações asiáticas; e (iii) um módulo de redes neurais ligado a NIR, FTIR, sensores e produtos como azeites, queijos e chás em contextos europeus. Essa estrutura modular confirma empiricamente a existência de subcampos especializados, nos quais combinações específicas de instrumentos e algoritmos funcionam como plataformas tecnológicas integradas para autenticação de produtos com Indicações Geográficas.

A Figura 9 mostra a rede de coocorrência entre produtos, técnicas analíticas e algoritmos, com tamanhos e cores de nós proporcionais às métricas de centralidade, o que torna visíveis os núcleos algorítmicos e as pontes entre módulos temáticos.

![Rede de coocorrência entre produtos, técnicas analíticas e algoritmos de machine learning, com tamanhos e cores de nós proporcionais às métricas de centralidade.](2-FIGURAS/network_centrality_metrics.png){#fig:network_centrality width="80%"}
...
A consolidação de ML em sistemas de IGs exige, por fim, um ecossistema de suporte que articule infraestrutura laboratorial, competências em ciência de dados e governança de dados. A integração do conhecimento empírico das comunidades produtoras com evidências computacionais, observada em apenas 3% dos estudos, é decisiva para a legitimidade social dos modelos [@Huera-Lucero2025]. No contexto brasileiro, marcos legais como a Lei 15.068/2024 (Lei Paul Singer) podem fomentar a criação de Empreendimentos Econômicos Solidários especializados em ML [@Brasil2024; @Mazzucato2013], desde que acompanhados por redes laboratoriais com protocolos harmonizados [@MAPA2020] e por arranjos de governança que definam claramente direitos de propriedade intelectual e mecanismos de repartição justa de benefícios derivados do conhecimento territorial.

## 4. Conclusão

Esta revisão de escopo mapeou sistematicamente o estado da arte da aplicação de Aprendizado de Máquina (ML) em Indicações Geográficas (IGs), confirmando a hipótese de que o campo está em rápida maturação e organizado em torno de eixos tecnológicos, geográficos e metodológicos bem definidos. 
A análise integrada do corpus de 148 estudos mostrou que a intersecção entre ML e certificação de origem configura um ecossistema de nichos especializados, em que a escolha do algoritmo e da plataforma analítica é condicionada pela matriz do produto, por seu valor de mercado e pelo estágio de consolidação da IG.

Os resultados da MCA, da análise de redes e da análise temporal convergem para uma mesma leitura: há um polo de consolidação, centrado em produtos europeus tradicionais (vinhos, azeites) associados a algoritmos clássicos (PLS-DA, SVM) e técnicas espectroscópicas consolidadas, e uma fronteira de inovação, que conecta produtos asiáticos emergentes (chás, plantas medicinais) a técnicas de alta resolução (metabolômica) e arquiteturas de Deep Learning. Essa dualidade indica uma transição gradual de um paradigma confirmatório, focado em validar origens já consolidadas, para um paradigma exploratório, voltado à descoberta de novos marcadores e à agregação de valor competitivo.

À luz desse panorama, recomenda-se que pesquisas futuras priorizem a integração de dados multimodais, combinando informações químicas, espectrais, isotópicas, sensoriais e contextuais em modelos de ML capazes de capturar a complexidade territorial,  o desenvolvimento e a adoção de modelos interpretáveis, ou complementados por técnicas de explicabilidade, que apoiem decisões regulatórias e fortaleçam a confiança de produtores e consumidores e a aplicação sistemática dessas abordagens a IGs de países em desenvolvimento, como o Brasil, alinhando estratégias de certificação territorial a agendas de desenvolvimento regional sustentável e de proteção da propriedade intelectual coletiva.
